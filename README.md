...
- **5-window, GIMP-style layout** (each a separate top-level window):
  ...
  - **Environment (HRRR)** — fetches the hourly **HRRR 0 °C freezing level** (idx byte-range pull of the single `HGT : 0C isotherm` GRIB message from the `noaa-hrrr-bdp-pds` S3 bucket) and runs **vault detection**: each radar cell's ≥ 50 dBZ echo tower is measured against the 0 °C level, and a tower punching deep above it flags the cell's **overshooting top** (the OT flag is derived here — the GOES anvil-depth OT was removed as unreliable; cloud-top temperatures remain from the Satellite window).
